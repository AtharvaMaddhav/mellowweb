...
// import { fetchPosts, fetchUserData, toggleLike, reportPost, addPost, deleteExpiredPost } from "../../services/postService.js";
...
//         console.log(`Deleting ${postsToDelete.length} expired posts from database`);
...
//         setPosts(prevPosts => prevPosts.filter(post => !deletedIds.includes(post.id)));
//       }
//     };
    
//     // Set up interval to check every few seconds
//     const interval = setInterval(checkExpiredPosts, 5000);
  ...
  useEffect(() => {
    const checkExpiredPosts = async () => {
      ...
      if (reportedPostsToDelete.length > 0) {
        console.log(`Deleting ${reportedPostsToDelete.length} posts that exceed report threshold`);
            ...
            >
              ...
              <div className="p-4">
                ...
                <div className="mt-4 flex items-center justify-between pt-3 border-t">
                  <button 
                    onClick={() => handleLike(post.id)}
                    className={`flex items-center space-x-1 ${isLiked ? 'text-blue-500' : 'text-gray-500'} hover:text-blue-600 transition cursor-pointer`}
                  >
                    <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill={isLiked ? "currentColor" : "none"} viewBox="0 0 24 24" stroke="currentColor">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z" />
                    </svg>
                  ...
                  <div className="flex items-center">
                    ...
                    <button 
                      ...
                      // Disable if user already reported
                      disabled={post.reports?.includes(currentUser?.uid)}
                    >
                      {post.reports?.includes(currentUser?.uid) ? 'Reported' : 'Report'}
                    </button>
                  </div>
                </div>
              </div>
