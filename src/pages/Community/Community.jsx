...
import { Users, Plus, Trophy, BookOpen, Heart, Dumbbell, Brain, Filter, Compass, Sparkles } from "lucide-react";
...
import { getRecommendedCommunities, getUserInterests } from "../../services/recommendCommunities.js";
...
const Community = () => {
  ...
  const [aiRecommendedCommunities, setAiRecommendedCommunities] = useState([]);
  ...
  useEffect(() => {
    const fetchUserInterests = async () => {
      if (currentUserId) {
        try {
          const interests = await getUserInterests(currentUserId, db);
  ...
  useEffect(() => {
    const fetchAiRecommendations = async () => {
      ...
      try {
        ...
        const joinedIds = userCommunities.map(c => c.id);
        const recommendedIds = await getRecommendedCommunities(
          userInterests,
          communities,
          joinedIds
        );
        ...
        const recommended = recommendedIds
          ...
          .filter(Boolean)
          .slice(0, 6); // Show top 6 AI recommendations
        
        setAiRecommendedCommunities(recommended);
      } catch (error) {
        console.error("Error fetching AI recommendations:", error);
        // Fallback to suggested communities
        setAiRecommendedCommunities(suggestedCommunities);
      } finally {
  ...
  }, [currentUserId, communities, userCommunities, userInterests]);
  ...
  const matchesSearchQuery = (community, query) => {
    const name = normalizeSearchText(community.name);
  ...
  useEffect(() => {
    ...
    const debounceTimeout = setTimeout(async () => {
      ...
      try {
        ...
        if (!active) return;
        setSearchResults(fetchedCommunities.filter((community) => matchesSearchQuery(community, query)));
      } catch (err) {
  ...
  const CommunityCard = ({ community, isAiRecommended = false }) => {
    const isUserMember = community.members && community.members.includes(currentUserId);
    
    return (
      <div 
        onClick={() => handleViewCommunity(community)}
        className="bg-[#252525] rounded-xl p-5 hover:bg-[#2A2A2A] transition-all border border-gray-700 hover:border-purple-500 shadow-lg cursor-pointer relative"
      >
  ...
  return (
    <div className="flex h-screen bg-black text-white">
      ...
      <div className="flex-1 p-6 overflow-y-auto overflow-x-hidden h-screen">
            ...
            <div className="bg-[#111] border border-gray-800 rounded-xl p-5">
              <div className="flex items-center justify-between mb-4">
                <div>
                  ...
                  <p className="text-sm text-gray-400 mt-1">Showing communities whose name words start with "{searchQuery.trim()}".</p>
                </div>
              </div>
              ...
              {searchLoading ? (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                  {Array(3).fill(0).map((_, index) => (
                    <div key={index} className="bg-[#222] animate-pulse rounded-xl h-24"></div>
                  ))}
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                  {searchResults.map((community) => (
                    <button
                      key={community.id}
                      onClick={() => handleViewCommunity(community)}
                      className="w-full text-left bg-[#181818] border border-gray-800 rounded-xl p-4 hover:border-purple-500 transition-all"
                    >
                      <h3 className="text-white font-semibold truncate">{community.name}</h3>
                      <p className="text-gray-400 text-sm truncate mt-1">{community.tagline || community.description || "No description"}</p>
                    </button>
                  ))}
                </div>
              ) : (
                <div className="rounded-xl border border-dashed border-gray-700 bg-[#0f0f0f] p-6 text-center text-gray-400">
                  <p className="text-white font-medium">No matching communities found.</p>
                  <p className="text-sm mt-2">Search only matches community name words from the beginning.</p>
                </div>
              )}
            </div>
          </div>
        ...
        {/* AI Recommended Communities Section - NEW */}
        {currentUserId && aiRecommendedCommunities.length > 0 && (
          <div className="mb-8">
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-xl font-semibold text-white flex items-center gap-2">
                {/* <Sparkles size={18} className="text-purple-400" /> */}
                Recommendeded Communities
              </h2>
              {userInterests.length > 0 && (
                <div className="flex gap-2">
                  {userInterests.slice(0, 3).map((interest, idx) => (
                    <span key={idx} className="text-xs px-2 py-1 bg-purple-900/30 rounded-md text-purple-300">
                      {interest}
                    </span>
                  ))}
                </div>
            ...
            {loadingRecommendations ? (
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {Array(3).fill(0).map((_, index) => (
                  <div key={index} className="bg-[#222] animate-pulse p-5 rounded-xl h-40"></div>
                ))}
              </div>
            ) : (
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {aiRecommendedCommunities.map((community) => (
                  <CommunityCard key={community.id} community={community} isAiRecommended={true} />
                ))}
              </div>
            )}
          </div>
