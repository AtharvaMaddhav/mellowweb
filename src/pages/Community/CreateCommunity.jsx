...
const CreateCommunity = ({ onComplete }) => {
  ...
  const handleImageChange = (e) => {
    const file = e.target.files[0];
    if (file) {
      setNewCommunity(prev => ({ ...prev, image: file }));
      const reader = new FileReader();
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (newCommunity.image) {
        ...
        formData.append('file', newCommunity.image);
      ...
      // UPDATED: Changed collection name to 'Communities' (Capitalized)
      const docRef = await addDoc(collection(db, 'Communities'), communityData);
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50 overflow-y-auto">
      <div className="max-w-3xl w-full bg-[#111] rounded-xl p-6 shadow-xl border border-gray-800 max-h-[90vh] overflow-y-auto">
        <div className="flex justify-between items-center mb-6">
          ...
          <button onClick={onComplete} className="p-2 rounded-full hover:bg-[#333] transition-colors">
            <X size={20} className="text-gray-400" />
          </button>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <div>
            <label className="block text-gray-300 text-sm font-medium mb-2">Community Name*</label>
            <input
              type="text"
              name="name"
              value={newCommunity.name}
          ...
          <div>
            <label className="block text-gray-300 text-sm font-medium mb-2">Community Image</label>
            <div className="flex items-center space-x-4">
              <div 
                className="w-24 h-24 bg-[#252525] border border-dashed border-gray-600 rounded-lg flex items-center justify-center cursor-pointer overflow-hidden"
                onClick={() => document.getElementById('community-image').click()}
          ...
          <div className="flex justify-end gap-4 pt-4">
            ...
            <button
              type="submit"
              className="bg-gradient-to-r from-blue-600 to-purple-600 hover:from-blue-700 hover:to-purple-700 text-white px-5 py-2.5 rounded-lg font-medium transition-all shadow-lg disabled:opacity-70 disabled:cursor-not-allowed"
              disabled={isSubmitting}
            >
              {isSubmitting ? 'Creating...' : 'Create Community'}
            </button>
