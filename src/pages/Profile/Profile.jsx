...
const Profile = () => {
  ...
  // New function to fetch goals where user is owner or member
  const fetchUserGoals = async (targetUserId) => {
    try {
      const goalsRef = collection(db, "goals");
      
      // Create a query for goals where the user is the owner (userId field)
      const userOwnerQuery = query(
        goalsRef, 
        where("userId", "==", targetUserId)
      );
      
      // Create a query for goals where the user is a member (in members array)
      ...
      const [ownerSnapshot, memberSnapshot] = await Promise.all([
        getDocs(userOwnerQuery),
        getDocs(userMemberQuery)
      ]);
      
      // Process goals where user is owner
      const ownerGoals = [];
      ...
      // Process goals where user is a member
      const memberGoals = [];
      memberSnapshot.forEach((doc) => {
        // Check if this goal is already added as an owner goal to avoid duplicates
        const isAlreadyAdded = ownerGoals.some(goal => goal.id === doc.id);
      ...
      const allUserGoals = [...ownerGoals, ...memberGoals];
      
      console.log(`Found ${allUserGoals.length} goals for user ${targetUserId}`);
      setMyGoals(allUserGoals);
    ...
    if (!goals || goals.length === 0) {
      return (
        <div className="flex flex-col items-center justify-center min-h-40 py-12 text-center">
          ...
          <h3 className="text-xl text-gray-200 mb-2">No Goals Yet</h3>
        ...
        {goals.map((goal) => (
          <div key={goal.id} className="bg-gray-700 rounded-lg p-4">
            ...
            <div className="flex justify-between items-center text-xs text-gray-400">
              <div>Members: {goal.members?.length || 1}</div>
              <div>
  ...
  const renderInterestContent = () => {
    switch (activeSection) {
      ...
      case "goals":
        return renderGoalGrid(myGoals, goalsLoading);
      default:
        return null;
    }
  };
